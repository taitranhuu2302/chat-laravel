...
$(function () {
  ...
  Echo.channel("create-room.".concat(userId)).listen('CreateRoomEvent', function (data) {
    ...
    if (data.room.room_type === 'GROUP_ROOM') {
      toastify_js__WEBPACK_IMPORTED_MODULE_1___default()({
        text: "Ban \u0111\u01B0\u1EE3c m\u1EDDi v\xE0o ph\xF2ng chat ".concat(data.room.name),
        duration: 3000,
        newWindow: true,
        close: true,
        gravity: "top",
  ...
  $('#btn-add-friend-to-room').click(function (e) {
    ...
    if (friendObj) {
      ...
      if (listFriendCreateRoom.length < 4) {
        var html = "\n                    <img data-user-id=".concat(friendObj.user.id, " class=\"avatar-user-to-room cursor-pointer w-10 h-10 border-2 border-white rounded-full dark:border-gray-800\"\n                        src=\"").concat(friendObj.user.avatar, "\" alt=\"\">\n                 ");
        $('#list-avatar-user-to-room').prepend(html);
      }

      $('#list-user-in-room-count').text(listFriendCreateRoom.length);
      email.val('');
      var avatarUser = $('.avatar-user-to-room');
      avatarUser.unbind('click');
      avatarUser.click(function (e) {
        var userId = $(this).attr('data-user-id');
        $(this).remove();
        listFriendCreateRoom = listFriendCreateRoom.filter(function (item) {
          return item != userId;
        });
        $('#list-user-in-room-count').text(listFriendCreateRoom.length);
      });
  ...
  $('#btn-create-group').click(function (e) {
    ...
    axios.post('/room/create-room-group', {
      ...
      var wrapperListAvatar = $('#list-avatar-user-to-room');
