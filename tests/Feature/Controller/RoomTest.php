...
class RoomTest extends TestCase
{
    ...
    public function test_add_member_to_group()
    {
        ...
        $response = $this->json('POST', '/room/add-member-group', [
            'roomId' => $room->id,
            'email' => $userOne->email
        ]);
    ...
    public function test_return_error_if_add_member_exists_in_group()
    {
        $user = User::factory()->create();
        $userOne = User::factory()->create();
        $this->actingAs($user);
        ...
        $this->assertEquals('Người dùng đã tồn tại trong phòng', $response->json('message'));
    }
